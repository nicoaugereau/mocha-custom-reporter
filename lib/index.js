...
function pugToHtml(data, output, options) {
  ...
  switch(options.infos) {
    case 'summary':
      ...
      testsListName = 'Liste des tests'
      break
    case '':
      testsListName = 'Liste des tests en échec'
      break
    default:
  ...
  chart_passed = `#2cd860 0, #2cd860 ${pass_percent}%,`
  chart_failed = `#de3f3f 0, #de3f3f ${( Number(parseFloat(pass_percent)) + Number(parseFloat(fail_percent)) ).toFixed(4)}%,`
  // skipped -> pending
  chart_pending = `#fdb563 0, #fdb563 ${( Number(parseFloat(pass_percent)) + Number(parseFloat(fail_percent)) + Number(parseFloat(skip_percent)) ).toFixed(4)}%,`
  // pendinf -> skipped
  chart_skipped = `#666 0, #666 ${( Number(parseFloat(pass_percent)) + Number(parseFloat(fail_percent)) + Number(parseFloat(skip_percent)) + Number(parseFloat(pending_percent)) ).toFixed(4)}%`
