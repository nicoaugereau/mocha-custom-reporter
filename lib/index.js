...
function pugToHtml(data, output, options) {
  ...
  let now = new Date()
    ...
    if (pending_percent != 0 ) { 
      chart_pending = `#666 0, #666 ${( Number(parseFloat(pass_percent)) + Number(parseFloat(fail_percent)) + Number(parseFloat(skip_percent)) + Number(parseFloat(pending_percent)) ).toFixed(4)}%`
    }
  ...
  var html = fn({ 
    ...
    pending_numerator: data.stats.pending,
